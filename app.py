...
def fetch_and_calculate_features(market_name):
    ...
    for i in range(0, len(all_tickers), batch_size):
        ...
        try:
            # 🛡️ 教練修正：auto_adjust=True (使用還原收盤價，避開除權息暴跌的騙線)
            ...
            for ticker in batch:
                try:
                    ...
                    # 🛡️ 教練修正：紀錄個別股票運算錯誤，但繼續跑下一檔
                    print(f"計算 {ticker} 指標時發生錯誤: {e}")
                    continue
        except Exception as e:
            print(f"批次下載資料時發生錯誤: {e}")
            pass
...
if st.button("開始全面掃描", type="primary"):
    ...
    with st.status(f"🔍 啟動 {market} 運算中 (若無快取約需 1-2 分鐘)...", expanded=True) as status:
        ...
        if df_all.empty:
            ...
            st.error("目前無法取得數據，請稍後檢查網路或終端機錯誤訊息。")
            st.stop()
    ...
    # 🛡️ 教練修正：將波浪號(~)改為連字號(-)，完美解決 Streamlit Markdown 刪除線 Bug
